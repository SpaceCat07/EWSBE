...
// --- MQTT / WiFi config (replace with your values) ---
...
void mqttSetup() {
  ...
  // accept broker as host or with tcp:// prefix
  String broker = MQTT_BROKER;
  if (broker.startsWith("tcp://")) broker = broker.substring(6);
  mqttClient.setServer(broker.c_str(), MQTT_PORT);
}

bool mqttReconnect(const char* clientId) {
  if (mqttClient.connected()) return true;
  bool ok;
  if (strlen(MQTT_USER) == 0) ok = mqttClient.connect(clientId);
  else ok = mqttClient.connect(clientId, MQTT_USER, MQTT_PASS);
  if (!ok) {
    delay(1000);
    return false;
  }
...
void mqttPublishSensorData(unsigned long waktu, float suhu, float lembap, float tekanan, float ketinggian,
                           int co2, float jarak, float angin, float arahAngin,
                           float busVoltage, float current_mA, float voltSensor, float rain) {
  ...
  doc["jarak"] = jarak;
  doc["angin"] = angin;
  doc["arahAngin"] = arahAngin;
  doc["busVoltage"] = busVoltage;
  doc["current_mA"] = current_mA;
  doc["voltSensor"] = voltSensor;
  doc["rain"] = rain;

  char payload[512];
  size_t n = serializeJson(doc, payload, sizeof(payload));
  if (n > 0) mqttClient.publish(MQTT_TOPIC, payload);
}
...
void loop() {
  ...
  String clientId = "ESP32-" + String((uint32_t)ESP.getEfuseMac(), HEX);
  if (!mqttClient.connected()) mqttReconnect(clientId.c_str());
  mqttClient.loop();
  mqttPublishSensorData(waktu, suhu, lembap, tekanan, ketinggian,
                        co2, jarak, angin, arahAngin,
                        busVoltage, current_mA, voltSensor, rain);
